(() => {
  ...
  function select(tab){unload();selected=tab.dataset.demo;tabs.forEach(t=>{t.setAttribute('aria-selected',String(t===tab));t.tabIndex=t===tab?0:-1;});stage.setAttribute('aria-labelledby',tab.id);poster.src=`../assets/digital-twins/demo-${selected}-preview.webp?v=20260917-2`;poster.alt={'01':'3D earthwork model with photo camera positions','02':'3D property model with photo camera positions','03':'3D industrial building model with photo camera positions'}[selected];document.getElementById('standaloneDemo').href=`../assets/digital-twins/viewer.html?demo=${selected}`;track('select');}
  ...
  const requested=tabs.find(tab=>tab.dataset.demo===new URLSearchParams(location.search).get('demo'));
