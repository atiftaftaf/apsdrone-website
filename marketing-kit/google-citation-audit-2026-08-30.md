...
| LinkedIn company page | Public page was rechecked on August 30, 2026 and still says `Houston, Texas`, headquarters `Houston, Texas`, and primary location `Houston, Texas 77024, US`; website is still `http://apsdrone.com`. The authenticated Atif Taftaf profile is not an admin; the page returned an explicit no-access response. Atif now follows the page, and a truthful APS Drone founder/Part 107 role is prepared but not publicly saved. | Inconsistent; admin recovery in progress | After action-time approval, save the prepared APS Drone experience, then use LinkedIn's official Request admin access flow and current-admin/support review; never create a duplicate company page |
...
| Yelp Business | Public and owner-directory searches found no APS Drone listing; a hidden-address record was prepared with the canonical phone, website and three truthful categories. At 3:21 PM Central, Yelp sent a four-digit SMS code to `(832) 938-9570` and is waiting on the code-entry screen. | Prepared; SMS code pending | Enter the owner-provided code, finish the claim, then verify that only city/state/ZIP are public |
...
- Yelp's public Dallas search and official owner-directory search both failed to find APS Drone. The new hidden-address record advanced to the SMS code-entry screen after Yelp sent a four-digit code to the canonical business number; no second record should be created.
...
- LinkedIn confirmed that the signed-in `Atif Taftaf` profile lacks admin access to company ID `74151446`. The profile now follows APS Drone, and the required current APS Drone experience is prepared with DFW wording, 2021 start year, self-employed/hybrid status and network broadcast disabled; its final public Save remains action-time gated.
- Apple and LinkedIn remain fail-closed until the owner completes the Apple sign-in and approves the prepared LinkedIn public profile change without creating duplicate identities.
