...
| LinkedIn company page | The obsolete Houston company page was deactivated by the owner and now returns `This LinkedIn Page isn't available`. The truthful `Founder & FAA Part 107 Drone Pilot` APS Drone role remains saved on Atif's profile with DFW wording and network broadcasting disabled. A fresh Company-page creation attempt reached LinkedIn's official setup but was blocked by `Feature not available` because the personal account is too new; LinkedIn instructs waiting at least one day. | Old page removed; new-page creation temporarily platform-blocked | Retry the official Company-page creation flow after the one-day account-age hold; create one canonical DFW page only |
...
| Apple Business Connect | Official Apple Business sign-in is open; no Apple Account is authenticated and no owned place is visible | Free path available; Apple owner sign-in required | Sign in with the owner's Apple Account, then search before creating/claiming one service-area place; never expose a private unit address |
| Yelp Business | The free APS Drone claim passed SMS verification under `apsdronetx@gmail.com`; the paid Yelp Ads trial was explicitly declined. The service-area address is configured as hidden, and Dallas, Arlington, Plano, McKinney, Celina and Fort Worth are saved as service areas. Yelp confirms the business is claimed but will not appear publicly until moderator approval. | Claimed; Yelp moderation pending | Monitor the approval notice, then verify the public listing does not expose the street address and add real project photos |

## LinkedIn correction package

The public company page is `https://www.linkedin.com/company/aps-drone/`. The current in-app browser is not signed into the existing company administrator account. “Continue with Google” indicated that it would create an account, so it was not clicked.
...
- The obsolete LinkedIn page is no longer publicly available after owner deactivation. A replacement canonical DFW page is still needed once LinkedIn's one-day new-account restriction clears.
...
- Apple Business opened the official Apple Business sign-in screen and requires the owner's Apple Account credentials before any free Maps claim can be inspected or created.
- Yelp's free SMS claim completed successfully under the APSDroneTX account. The paid Ads trial was declined, the street address remains configured as hidden, and six DFW service areas are saved. Yelp's owner dashboard confirms the claim and states that publication is waiting for moderator approval; no second record should be created.
...
- LinkedIn's obsolete company ID `74151446` now resolves to the unavailable-page screen after owner deactivation. Atif's current APS Drone experience remains server-confirmed saved with DFW wording, 2021 start year, self-employed/hybrid status and network broadcast disabled. The official new Company-page flow was attempted immediately, but LinkedIn displayed `Feature not available` and instructed waiting at least one day because the personal account is recent.
- Apple remains fail-closed until the owner completes Apple Account sign-in and any 2FA. LinkedIn should be retried only after its stated one-day account-age hold; no duplicate identity or alternate company page should be created in the meantime.
