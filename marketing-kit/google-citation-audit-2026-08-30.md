...
| LinkedIn company page | Public page was rechecked on August 30, 2026 and still says `Houston, Texas`, headquarters `Houston, Texas`, and primary location `Houston, Texas 77024, US`; website is still `http://apsdrone.com`. Atif now follows the page and the truthful `Founder & FAA Part 107 Drone Pilot` APS Drone role was publicly saved with DFW wording and network broadcasting disabled. LinkedIn still returns explicit no-access and does not expose `Request admin access` in the company menu. | Inconsistent; current-admin or LinkedIn human review required | Ask the current super admin to grant access or use LinkedIn support/admin recovery; never create a duplicate company page |
...
- LinkedIn confirmed that the signed-in `Atif Taftaf` profile lacks admin access to company ID `74151446`. The profile now follows APS Drone, and the current APS Drone experience was server-confirmed saved with DFW wording, 2021 start year, self-employed/hybrid status and network broadcast disabled. After reload, the Page menu still omitted `Request admin access`, and the direct admin URL continued to return no access. Recovery now depends on a current super admin or LinkedIn support/human review.
- Apple remains fail-closed until the owner completes Apple Account sign-in and any 2FA. LinkedIn remains fail-closed at the company-admin layer until current-admin or support review; no duplicate identity or company page should be created.
